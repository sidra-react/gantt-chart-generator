...
bool checkscheduling(processesedf *processes,int n)    //to check scheduling
{
    ...
    for (int i = 0; i < n; i++) 
    {
       totaltime += float(processes[i].burst_time )/ float(processes[i].deadline);
	       }
...
settextstyle(9, HORIZ_DIR, 2);
    setcolor(GREEN);
    outtextxy(10, 10, "Checking if processes are schedulable...");
    delay(600);
    ...
    if(f==true){
     ...
     settextstyle(9, HORIZ_DIR, 2);
    setcolor(GREEN);
    outtextxy(10, 200, "0 Processes are schedulable, no tension:)");
  
    outtextxy(10, 250, "0 Press any key to get gantt chart");
